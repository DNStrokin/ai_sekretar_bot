...
async def _show_format_menu(message_or_obj, state: FSMContext, topic_id: int):
    ...
    async with session_maker() as session:
        ...
        text = (
            ...
            f"• <code>&lt;code&gt;код&lt;/code&gt;</code> → <code>код</code> (копируется при клике)\n"
            f"• <code>&lt;pre&gt;блок&lt;/pre&gt;</code> → блок кода (копируется)\n"
            f"• <code>&lt;blockquote&gt;цитата&lt;/blockquote&gt;</code> → <blockquote>цитата</blockquote>\n"
            f"• <code>&lt;a href='URL'&gt;ссылка&lt;/a&gt;</code> → <a href='https://t.me'>ссылка</a>\n\n"
